  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setRateValues([
          res.data?.hourFee || 5,
          res.data?.halfHourFee || 5,
          res.data?.fastestFee || 5,
        ]);
  ...
  return (
    <div className="modal-container z-30 absolute top-0 w-full left-0 h-full backdrop-blur-sm">
      <div
        ...
        onClick={onClose}
      ></div>
      <div className="modal-wrapper m-auto absolute w-full max-w-[500px] h-[470px] top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center bg-white shadow-lg shadow-[#00000050]">
        ...
        {!loading ? (
          <div className="button-group flex gap-8 mb-[20px] px-[30px] justify-center">
            <div className="border border-[#2c2e37] w-full py-[10px]">
              <div className="mb-[4px] text-center text-[24px]">
                {rateValues[0]}
              </div>
              <div className="mb-[4px] text-center text-[14px]">Sats/VB</div>
              <div className="mb-[4px] text-center text-[16px] font-semibold">
                SLOW
              </div>
            </div>
            <div className="border border-[#2c2e37] w-full py-[10px]">
        ...
        <div className="flex gap-8 px-[30px] items-center text-[14px] justify-center">
          {!showCustom && (
            <div
              className="px-[15px] py-[10px] text-center cursor-pointer text-[14px] font-semibold whitespace-nowrap border border-[#2c2e37]"
          ...
          {showCustom && <div>Sats/VB</div>}
        </div>
        <div className="my-[30px] text-[#00000070] text-center text-[16px] text-dark">
          Estimated fee: {feeRate} Sats/VB
        </div>
