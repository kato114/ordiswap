...
function ExchageRemoveLiquidity() {
  ...
  const [result, getResult] = useTokenOneAndTwo(
    lPAmount,
    tokenOne,
    tokenTwo,
    lPToken
  ...
  useEffect(() => {}, [result, lPAmount]);

  const [lpTokenList, setLpTokenList] = useState(
    poolList.map((pool) => {
      return {
        tick: pool.lp_token,
        token1: pool.token1,
  ...
  const lpTokenSend = (record, id) => {
    ...
    return (
      <>
        <TooltipComp
          content={`Receive ${token} from pool ${record.token1}/${record.token2} (${inscriptionId}) `}
        >
          <button
  ...
  const amountRender = (record) => {
    if (record.token_amount1)
      return (
        <span>{`${
          record.token1 === "BTC"
            ? record.token_amount1 / 1e8
            : record.token_amount1
        }/${
          record.token2 === "BTC"
            ? record.token_amount2 / 1e8
            : record.token_amount2
        }`}</span>
      );
    else return <></>;
  };
  ...
  const handleRemoveLiquidity = async () => {
    ...
    try {
      ...
      const tx_id = await unisatWallet.sendBitcoin(
        factoryWallet,
        calculateFee(feeRate).remove_liquidity_fee
      );
      const body = {
      ...
      if (data.status == "success") {
        messageApi.notifySuccess(
          "Remove liqudity order is successfully listed!"
        );
        await loadOrderList();
      } else {
        ...
        <div className="swap__form center-margin full-w-select">
          ...
          <ExchangeSelect
            amount={
              result
                ? tokenOne.tick == "BTC"
                  ? (result.token_amount1 / 1e8).toFixed(8)
                  : result.token_amount1
                : ""
            }
          ...
          <ExchangeSelect
            amount={
              result
                ? tokenTwo.tick == "BTC"
                  ? (result.token_amount2 / 1e8).toFixed(8)
                  : result.token_amount2
                : ""
            }
