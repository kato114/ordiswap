...
const reducer = (state, { type, payload }) => {
  switch (type) {
    case authActions.SET_THEME: {
      ...
      return {
        ...state,
        preferDark: true, //payload.preferDark
      };
    }
    case authActions.UPDATE_THEME: {
      localStorage.setItem("theme", true); // !state.preferDark
      // console.log('AAAAAAAAAAAAAAAAA');
      return {
        ...state,
        preferDark: true, // !state.preferDark
      };
    }
...
export function AuthStateProvider({ children }) {
  ...
  const checkConnect = async () => {
    return new Promise(async (res, rej) => {
      ...
      else currentNetwork = await window.unisat.requestAccounts();
  ...
  useEffect(() => {
    (async () => {
      const tokenIconList = await axios.get(tokenDataListApi, {
        header: {
          "x-access-token":
            "coinranking999e7dfea873fd6e36dbec0b290d1d80a23eba1cadf68654",
        },
      });
  ...
  useEffect(() => {
    ...
    authDispatch({
      type: authActions.SET_THEME,
      payload: { preferDark: isdark },
    });
