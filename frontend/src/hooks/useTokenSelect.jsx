...
export function useTokenSelect(address) {
  ...
  const [tokenList, fetchTokenList] = useFetch(
    address != undefined ? tokenListApi + address : tokenListApi,
    { method: "get" }
  );
  const [poolTokenList, fetchPoolTokenList] = useFetch(
    address != undefined ? poolTokenListApi + address : poolTokenListApi,
    { method: "get" }
  );
  ...
  const [arrayPoolTokens, setArrayPoolTokens] = useState([
    { tick: "BTC", balance: 0 },
  ]);
