...
function Reward() {
  ...
  useEffect(() => {
    if (rewardsData) {
      setCreator(false);
      if (rewardsData.lp_holder_balance > 0) setCreator(true);
    }
  }, [address, rewardsData]);
  ...
  useEffect(() => {
    const parseList = async () => {
      ...
      const orders = orderList.filter(
        (row) => row.order_type == 5 && row.order_status != 99
      );
  ...
  const handleWithdraw = async () => {
    ...
    try {
      messageApi.notifyWarning(`Ordering Withdraw BTC`, 10);
      let tx_id;
      tx_id = await unisatWallet.sendBitcoin(
        factoryWallet,
        calculateFee(feeRate).withdraw_fee
      );
      const body = {
        ...
        withdraw_amount:
          rewardsData.admin_balance + rewardsData.lp_holder_balance,
      };
      ...
      {modalState.open && (
        <ReactPortal>
          <section className="modal__content">
            <p>
              {`Are you sure withdraw rewards with a service fee of ${
                calculateFee(feeRate).withdraw_fee / 1e8
              } BTC?`}
            </p>

            <div className="btn-group">
              <button className="btn w-full" onClick={handleWithdraw}>
        ...
        <div className="card-box">
          {admin && (
            <>
              ...
              <div className="items-center justify-center cell-box">
                <div className="flex items-center justify-center">
                  ...
                  <div className="whitespace-nowrap pl-5">
                    {(rewardsData.admin_balance / 1e8).toFixed(8) || "0"} BTC
                  </div>
                </div>
                {/* <button 
              ...
              {creator && <div className="d-divider"> </div>}
            </>
          )}
          {creator && (
            <>
              <h3 className="mt-[40px]">LP Creator Reward</h3>
                  ...
                  {rewardsData.lp_holder_details.map((row, index) => {
                    return (
                      <div className="flex">
                        ...
                        <div className="pl-5">
                          {(row.balance / 1e8).toFixed(8) || "0"} BTC
                        </div>
                      </div>
                    );
                  })}
              ...
              <>
                ...
                <div className="flex items-center mt-5 justify-center">
                  ...
                  <div className="pl-5 font-bold">
                    {(
                      (rewardsData.lp_holder_balance +
                        rewardsData.admin_balance) /
                      1e8
                    ).toFixed(8) || "0"}{" "}
                    BTC
                  </div>
                ...
                <button
                  className="px-[18px] py-[5px] text-[14px] bg-[#2c2e37] active:opacity-60 hover:opacity-80 h-fit mt-[30px]"
                  disabled={
                    !address ||
                    rewardsData.lp_holder_balance + rewardsData.admin_balance <=
                      0 ||
                    isDisabled
                  }
          ...
          {!admin && !creator && <p className="noreward">There is no reward</p>}
        </div>
